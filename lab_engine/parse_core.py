...
SYNONYMS = {
    ...
    "RDW": ["rdw", "rdw-cv", "гетерогенностьэритроцитовпообъему", "гетерогенностьэритроцитовпообъёму", "анизоцитоз эритроцитов"],
...
def extract_date(filename: str, text: str) -> str | None:
    ...
    # 1) Дата ЗАБОРА из текста — высший приоритет.
    for pat in [
    ...
    # 3) Последний резерв — дата получения/печати результата (МЕДСИ/Гемотест,
    #    где нет даты забора и даты в имени файла). Ниже приоритетом, чтобы НЕ
    #    смещать даты забора у уже работающих ДНКОМ/ЕМЦ-файлов.
    for pat in [
...
def detect_lab(filename: str, text: str) -> str:
    ...
    if "dvs" in n.lower() or "dobrynin" in n.lower():
        return "DNKOM (Клиника Герича)"
...
def is_garbage_name(name: str) -> bool:
    ...
    # Единица измерения внутри «имени» = обрывок интерпретации/комментария, не маркер.
...
# Референс отдельной строкой ПОД значением (ДНКОМ ВЭЖХ-МС: витамины и др.).
...
def _merge_orphan_refs(text: str) -> str:
    ...
    (ДНКОМ ВЭЖХ-МС). Инлайн-форматы не затрагивает (у них строк-«только-порогов» нет)."""
    lines = text.split("\n")
...
def parse_text(text: str) -> list[dict]:
    ...
    text = _merge_orphan_refs(text)  # референсы отдельной строкой (ВЭЖХ-МС)
...
def split_visits(text: str) -> list[tuple[str, str]]:
    ...
    # Якорь начала визита — «Ф.И.О.: ДОБРЫНИН» или «Заявка №»
    anchors = list(re.finditer(r"^[\s ]*Ф\.И\.О\.:\s*ДОБРЫНИН", text, re.MULTILINE | re.IGNORECASE))
    if len(anchors) < 2:
        return [(None, text)]
    # Разбиваем по якорям
    chunks = []
